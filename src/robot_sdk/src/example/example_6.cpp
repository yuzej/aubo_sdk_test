...
void Example_6::demo()
{
    ...
    aubo_robot_namespace::ToolInEndDesc toolDesc;  //Tools used to calibrate coordinates
    ...
    if(robotService.robotServiceCheckUserCoordinate(userCoord) == aubo_robot_namespace::InterfaceCallSuccCode)
    {
        std::cout<<"Define the coordinate system successfully."<<std::endl;
    }
    else
    {
        std::cerr<<"Defining the coordinate system failed."<<std::endl;
    }
    ...
    if(robotService.robotServiceJointMove(jointAngle, true) != aubo_robot_namespace::InterfaceCallSuccCode)  //关节运动至准备点
    {
        std::cerr<<"Movement to preparation point failed.　ret:"<<ret<<std::endl;
    }
    ...
    for(int i=0;i<10;i++)
    {
        ...
        position.y = 0.0;
        position.z = 0.0;
        //Keep the current pose moving through the linear motion to move the tool end point (toolDesc_1) to the specified position of the specified coordinate system (userCoord).
        if(robotService.robotMoveLineToTargetPosition(userCoord, position, toolDesc_1, true) != aubo_robot_namespace::InterfaceCallSuccCode)
        {
